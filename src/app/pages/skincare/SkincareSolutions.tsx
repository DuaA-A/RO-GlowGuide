...
import { skincareRoutines, skinTypes } from "../../data/skincare";
import { useLanguage } from "../../context/LanguageContext";
import { skincareRoutinesAr, skinTypesAr } from "../../data/skincare_ar";
    ...
    return (
        <div className="py-20">
            <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
                ...
                <SectionHeader
                    label={t("skincare.solutions.label")}
                    title={t("skincare.solutions.title")}
                    titleHighlight={t("skincare.solutions.titleHighlight")}
                    subtitle={t("skincare.solutions.subtitle")}
                />
                                ...
                                <div className={`flex flex-col md:flex-row ${i % 2 === 1 ? 'md:flex-row-reverse' : ''} ${isAr ? "md:flex-row-reverse" : ""} flex-none border-b border-warm-beige/30`}>
                                    ...
                                    <div className={`flex-1 p-4 md:p-6 flex flex-col justify-center ${isAr ? "text-right" : ""}`}>
                                        <div className={`flex items-center gap-2 mb-2 ${isAr ? "flex-row-reverse" : ""}`}>
                                            <span className="badge-skincare text-[9px] py-0.5 px-2">{isAr ? (skinTypesAr[routine.targetType]?.name ?? routine.targetType) : routine.targetType} {t("product.bestForSuffix")}</span>
                                            <div className={`flex items-center gap-3 text-[9px] uppercase tracking-wider text-taupe/70 font-bold ${isAr ? "mr-auto" : "ml-auto"} ${isAr ? "flex-row-reverse" : ""}`}>
                                                ...
                                                <span className={`flex items-center gap-1 md:flex hidden ${isAr ? "flex-row-reverse" : ""}`}><Clock className="w-3 h-3" />{t("label.dailyRoutine")}</span>
                                            </div>
                                        </div>
                                        <h3 className="font-heading text-espresso mb-1 text-xl md:text-2xl">{isAr ? skincareRoutinesAr[routine.id]?.name ?? routine.name : routine.name}</h3>
                                        <p className="text-espresso/80 text-sm md:text-base leading-relaxed line-clamp-2 italic">
                                            {isAr ? skincareRoutinesAr[routine.id]?.description ?? routine.description : routine.description}</p>
                                    </div>
                                </div>
                            ) : (
                                ...
                                <div className="flex md:grid md:grid-cols-2 lg:grid-cols-3 gap-4 pb-6 overflow-x-auto md:overflow-x-visible snap-x snap-mandatory scrollbar-none">
                                    {routine.steps.map((step, idx) => (
                                        ...
                                        >
                                            ...
                                            <h4 className="border-b border-warm-beige/30 pb-2 font-heading text-espresso text-base mb-2">{isAr ? skincareRoutinesAr[routine.id]?.steps[idx]?.name ?? step.name : step.name}</h4>
                                            <p className="text-sm text-espresso/80 leading-relaxed mb-3 flex-grow line-clamp-4">
                                                {isAr
                                                    ? skincareRoutinesAr[routine.id]?.steps[idx]?.description ?? step.description
                                                    : step.description}
                                            </p>
                ...
                >
                    ...
                    <h3 className="font-heading text-espresso mb-4">{isAr ? "هل أنتِ مستعدة لبناء رف منتجاتك؟" : "Ready to build your product shelf?"}</h3>
                    <p className="text-espresso/70 text-sm mb-7 max-w-lg mx-auto leading-relaxed">
                        {isAr ? "استكشفي كتالوج منتجات العناية بالبشرة المنتقى وابحثي عن التركيبات المُوصى بها في كل خطوة من خطوات الروتين." : "Explore our curated skincare product catalogue and find the exact formulas recommended in each routine step."}
                    </p>
